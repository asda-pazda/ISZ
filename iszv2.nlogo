...
to fix-turtles-energy
  let turtles-energy sum [energy] of turtles
  let difference turtles-energy - total-energy
  let correction difference / count turtles
  ask turtles [
    set energy energy - correction
  ]
end
...
to go
  ...
  let patches-with-collisions patches with [count turtles-here > 1]
  set number-of-collisions count patches-with-collisions
  if number-of-collisions > 0 [
    ; obliczamy ile energii z akumulatora przypada na jedna kolizje
    set energy-per-collision accumulated-energy / number-of-collisions
    set accumulated-energy 0
...
to split-energy-between-agents [agents]
  if collision-split-strategy = "equal-split"            [ equal-split agents ]
...
to equal-split [collided-agents]
  let number-of-collided-agents count collided-agents
  let energy-per-agent energy-per-collision / number-of-collided-agents
  ask collided-agents [
    set energy energy + energy-per-agent
  ]
end

to strongest-takes-all [collided-agents]
  let number-of-collided-agents count collided-agents
  let strongest-agent collided-agents with-max [energy]
  ; kiedy jest wiecej niz jeden agent z maksymalna energia
...
to strongest-takes-all-v2 [collided-agents]
  let number-of-collided-agents count collided-agents
  let strongest-agent collided-agents with-max [energy]
  ; kiedy jest wiecej niz jeden agent z maksymalna energia
  ifelse (count strongest-agent > 1) [
    ...
    let agents-energy sum [energy] of collided-agents

    let agent one-of strongest-agent
    ask agent [
      set energy agents-energy + energy-per-collision
      ask other collided-agents [
        die
      ]
    ]
  ]
...
to altruistic-split [collided-agents]
  let agents-energy sum [energy] of collided-agents
  let energy-to-split (agents-energy + energy-per-collision)
  let number-of-collided-agents count collided-agents
  let energy-per-agent (energy-to-split / number-of-collided-agents)
...
517
Total energy
accumulated-energy + sum [energy] of turtles
2
1
11
...
"" ""
PENS
"default" 1.0 0 -2674135 true "" "plot number-of-collisions"
...
"" ""
PENS
"turtles-energy" 1.0 0 -13840069 true "" "plot sum [energy] of turtles"
"accumulated-energy" 1.0 0 -2674135 true "" "plot accumulated-energy"
